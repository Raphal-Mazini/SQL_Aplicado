...
----------------------------- TABELA INFORMAÇÕES CIDADE ------------------------------------------------------------------------------------------------------------------------------
CREATE TABLE mc_cidade (
    cd_cidade  NUMBER GENERATED ALWAYS AS IDENTITY(START with 1 INCREMENT by 1),
    sg_estado  CHAR(2) NOT NULL,
...
ALTER TABLE mc_cidade ADD CONSTRAINT fk_mc_cidade_estado FOREIGN KEY ( sg_estado ) REFERENCES mc_estado ( sg_estado );



----------------------------TABELA INFORMAÇÕES BAIRRO ---------------------------------------------------------------------------------------------------------------------
CREATE TABLE mc_bairro (
    cd_bairro       NUMBER GENERATED ALWAYS AS IDENTITY(START with 1 INCREMENT by 1),
...
ALTER TABLE mc_bairro ADD CONSTRAINT fk_mc_bairro_cidade FOREIGN KEY ( cd_cidade ) REFERENCES mc_cidade ( cd_cidade );



---------------------------TABELA INFORMAÇÕES LOGRADOURO -----------------------------------------------------------------------------------------------------------------------------------------
CREATE TABLE mc_logradouro (
    cd_logradouro  NUMBER GENERATED ALWAYS AS IDENTITY(START with 1 INCREMENT by 1),
...
ALTER TABLE mc_logradouro ADD CONSTRAINT fk_mc_logradouro_bairro FOREIGN KEY ( cd_bairro ) REFERENCES mc_bairro ( cd_bairro );



---------------------------TABELA INFORMAÇÕES ENDEREÇO FUNCIONARIO --------------------------------------------------------------------------------------------------------------------------------------
...
ALTER TABLE mc_end_func ADD CONSTRAINT pk_mc_end_func_pk PRIMARY KEY ( cd_funcionario, cd_logradouro );
ALTER TABLE mc_end_func ADD CONSTRAINT fk_mc_end_func_logradouro FOREIGN KEY ( cd_logradouro ) REFERENCES mc_logradouro ( cd_logradouro );
ALTER TABLE mc_end_func ADD CONSTRAINT fk_mc_end_func FOREIGN KEY ( cd_funcionario ) REFERENCES mc_funcionario ( cd_funcionario );



--------------------------TABELA INFORMAÇÕES ENDEREÇO CLIENTE ----------------------------------------------------------------------------------------------------------------------------------
...
ALTER TABLE mc_end_cli ADD CONSTRAINT pk_mc_end_cli PRIMARY KEY ( nr_cliente, cd_logradouro_cli);
ALTER TABLE mc_end_cli ADD CONSTRAINT fk_mc_cliente_end FOREIGN KEY ( nr_cliente ) REFERENCES mc_cliente ( nr_cliente );
ALTER TABLE mc_end_cli ADD CONSTRAINT fk_mc_logradouro_end FOREIGN KEY ( cd_logradouro_cli ) REFERENCES mc_logradouro ( cd_logradouro );


---------------------TABELA INFORMAÇÕES CLIENTE (PF & PJ) ---------------------------------------------------------------------------------------------------------------------------------
CREATE TABLE mc_cliente (
    nr_cliente       NUMBER GENERATED ALWAYS AS IDENTITY(START with 1 INCREMENT by 1),
...
-- TABELA INFORMAÇÕES DEPARTAMENTO -------------------------------------------------------------------------------------------------------------------------------------
CREATE TABLE mc_depto (
    cd_depto  NUMBER GENERATED ALWAYS AS IDENTITY(START with 1 INCREMENT by 1),
...
----------------------- TABELA INFORMAÇÕES FUNCIONARIO -------------------------------------------------------------------------------------------------------------------------------------
CREATE TABLE mc_funcionario (
    cd_funcionario     NUMBER GENERATED ALWAYS AS IDENTITY(START with 1 INCREMENT by 1),
...
-------------------TABELA INFORMAÇÕES CLIENTE PESSOA FISICA-------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
ALTER TABLE mc_cli_fisica ADD CONSTRAINT fk_mc_cliente_fisica FOREIGN KEY ( nr_cliente ) REFERENCES mc_cliente ( nr_cliente );




------------------------TABELA INFORMAÇÕES PESSOA JURICA----------------------------------------------------------------------------------------------------------------------------------------------------
...
ALTER TABLE mc_cli_juridica ADD CONSTRAINT fk_mc_cliente_juridica FOREIGN KEY ( nr_cliente ) REFERENCES mc_cliente ( nr_cliente );



-- TABELA INFORMAÇÕES CATEGORIA PRODUTO --------------------------------------------------------------------------------------------------------------------------------------------
CREATE TABLE mc_categoria_prod (
    cd_categoria  NUMBER GENERATED ALWAYS AS IDENTITY(START with 1 INCREMENT by 1),
...
COMMENT ON COLUMN mc_categoria_prod.ds_categoria IS
    'Essa coluna irá armazenar descrição da categoria de produtos da Melhores Compras. Cada categoria tem uma  descrição única e serve para organizar os produtos em categorias simliares, melhorando a tomada de decisão.';
...
---------------------TABELA INFORMAÇÕES PRODUTOS----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE TABLE mc_produto (
    cd_produto         NUMBER GENERATED ALWAYS AS IDENTITY(START with 1 INCREMENT by 1),
...
ALTER TABLE mc_produto ADD CONSTRAINT fk_mc_sub_categoria_prod FOREIGN KEY ( cd_categoria ) REFERENCES mc_categoria_prod ( cd_categoria );





-- TABELA INFORMAÇÕES SERVIÇO ATENDIMENTO AO CLIENTE
CREATE TABLE mc_sgv_sac (
    nr_sac                    NUMBER GENERATED ALWAYS AS IDENTITY(START with 1 INCREMENT by 1),
    nr_cliente                NUMBER(10) NOT NULL,
...
ALTER TABLE mc_sgv_sac ADD CONSTRAINT fk_mc_cliente_sgv_sac FOREIGN KEY ( nr_cliente ) REFERENCES mc_cliente ( nr_cliente );
ALTER TABLE mc_sgv_sac ADD CONSTRAINT fk_mc_funcionario_sgv_sac FOREIGN KEY ( cd_funcionario ) REFERENCES mc_funcionario ( cd_funcionario );
ALTER TABLE mc_sgv_sac ADD CONSTRAINT fk_mc_produto_sgv_sac FOREIGN KEY ( cd_produto ) REFERENCES mc_produto ( cd_produto );



-- TABELA INFORMAÇÕES VIDEOS PRODUTOS
...
ALTER TABLE mc_sgv_produto_video ADD CONSTRAINT pk_mc_produto_imagem PRIMARY KEY ( nr_sequencia, cd_produto );
ALTER TABLE mc_sgv_produto_video ADD CONSTRAINT fk_mc_produto_imagem FOREIGN KEY ( cd_produto ) REFERENCES mc_produto ( cd_produto );
ALTER TABLE mc_sgv_produto_video ADD CONSTRAINT fk_mc_categoria_video FOREIGN KEY ( cd_categoria ) REFERENCES mc_categoria_prod ( cd_categoria );
...
-- TABELA INFORMAÇÕES VISUALIZAÇÃO VIDEO ------------------------------------------------------------------------------------------------------------------------------------------
CREATE TABLE mc_sgv_visualizacao_video (
    cd_visualizacao_video  NUMBER GENERATED ALWAYS AS IDENTITY(START with 1 INCREMENT by 1),
    nr_cliente             NUMBER(10) NOT NULL,
    cd_produto             NUMBER(10) NOT NULL,
    nr_sequencia           NUMBER(3) NOT NULL,
...
ALTER TABLE mc_sgv_visualizacao_video ADD CONSTRAINT fk_mc_sgv_prod_video_visual FOREIGN KEY ( nr_sequencia, cd_produto ) REFERENCES mc_sgv_produto_video ( nr_sequencia, cd_produto );
ALTER TABLE mc_sgv_visualizacao_video ADD CONSTRAINT fk_mc_cliente_visualiz_video FOREIGN KEY ( nr_cliente ) REFERENCES mc_cliente ( nr_cliente );
